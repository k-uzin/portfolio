$(function () {
    ...
    window.addEventListener('scroll', () => {
        ...
        if (scrollBar > 0) {
            header.classList.add('on'); /* .classList.add('on') > .on 아니고 점 안 붙인 채로 기입 */
        } else {
    ...
    $(document).ready(function () {
        $(document).scroll(function () {
            ...
            let scroll_percent = (scroll_pos / (document_height - $(window).height())) * 100;

            console.log(scroll_percent);

            if ($(window).width() <= 767) {
                if (scroll_percent >= 23 && scroll_percent < 83 || scroll_percent >= 99) {
                ...
                if (scroll_percent >= 40 && scroll_percent < 73 || scroll_percent >= 96) {
                    $("body").css('background-color', '#000000');
                ...
                if (scroll_percent >= 25 && scroll_percent < 65 || scroll_percent >= 91) {
                    $("body").css('background-color', '#000000');
    ...
    //body 배경색에 따른 header 글자 색상 변경
    $(document).scroll(function () {
        let bodyBackgroundColor = $("body").css('background-color');
        let header = $("header");
